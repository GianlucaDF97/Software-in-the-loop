 ...
 *  ------------------------------------------------------------------------- 
 *
 * Created: Thu Oct 14 16:04:04 2021
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME outputStream_v1
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    uint8_T *txBuf = (uint8_T *) ssGetOutputPortRealSignal(S, 0);
    uint32_T *length = (uint32_T *) ssGetOutputPortRealSignal(S, 1);

    outputStream_v1_Outputs_wrapper(u0, txBuf, length);
