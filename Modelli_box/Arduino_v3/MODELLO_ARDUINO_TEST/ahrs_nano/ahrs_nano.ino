...
SoftwareSerial ahrsSerial(10, 11); // RX, TX
...
void setup() {
  Serial.begin(38400);
  ahrsSerial.begin(38400);
  Serial.print("Attendo 2 secondi...");
  delay(2000);
  Serial.println("[ OK ]");

  while (ahrsSerial.available()<=0) {
    Serial.print("Richiedo le misure all'AHRS ... ");
    ahrsSerial.write(go_to_meas,5);
    Serial.println("[ OK ]");
    delay(1000);
  }
...
void riceviSMS() {
  ...
  if ( ahrsSerial.available() > 0 ) {

    tmp_buff[k-3] = ahrsSerial.read();
    bytes_received++;
