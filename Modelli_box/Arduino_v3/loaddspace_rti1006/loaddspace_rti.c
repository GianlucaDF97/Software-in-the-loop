...
#define RTB_STRUCTURE_NAME             loaddspace_B
#define RTP_STRUCTURE_NAME             loaddspace_P
...
static void rti_4201SB1I0S0(rtk_p_task_control_block task)
{
  ...
  /* Task code. */
  {
    ...
    /* {S!d8532}RateTransition: '<Root>/Rate Transition' */
    switch (loaddspace_DW.RateTransition_write_buf) {
     case 0:
      loaddspace_DW.RateTransition_read_buf = 1;
      break;

     case 1:
      loaddspace_DW.RateTransition_read_buf = 0;
      break;

     default:
      loaddspace_DW.RateTransition_read_buf =
        loaddspace_DW.RateTransition_last_buf_wr;
      break;
    }

    if (loaddspace_DW.RateTransition_read_buf != 0) {
      for (i = 0; i < 41; i++) {
        loaddspace_B.RateTransition_e[i] =
    ...
    /* {E!d8532}End of RateTransition: '<Root>/Rate Transition' */

    /* {S!d8529}S-Function (rti_commonblock): '<S6>/S-Function1' */
    loaddspac_FunctionCallSubsystem();

    /* {E!d8529}End of Outputs for S-Function (rti_commonblock): '<S6>/S-Function1' */
  }
...
__INLINE void rti_mdl_sample_input(void)
{
  ...
  /* dSPACE I/O Board DS4002 #1 Unit:TIMER */
  {
    ...
    ds4002_pwm2d_overl(DS4002_1_BASE, 1, periodsNumOf, &actualPeriodsNumOf,
                       &loaddspace_B.SFunction_o1, &loaddspace_B.SFunction_o2);
  }
