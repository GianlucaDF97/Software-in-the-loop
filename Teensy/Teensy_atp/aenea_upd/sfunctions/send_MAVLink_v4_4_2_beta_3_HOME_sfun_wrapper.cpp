 ...
 *
 */
void send_MAVLink_v4_4_2_beta_3_HOME_sfun_Start_wrapper(void)
{
 ...
 *
 */
void send_MAVLink_v4_4_2_beta_3_HOME_sfun_Outputs_wrapper(const uint8_T *iflogic,
			const uint32_T *time_in,
			const uint8_T *AP_SI,
			const uint16_T *GC_Req,
			const real_T *P_val,
			const uint16_T *WP_info,
			const real_T *WP_param,
			...
			const real_T *attitude,
			const real_T *lat_long_alt,
			const int16_T *velGPS,
			const uint16_T *hdg,
			const real_T *VFR_speed,
			const int16_T *VFR_H,
			const uint16_T *VFR_T,
			const uint16_T *Ser_data,
			const uint16_T *priori_in,
			const real_T *P_refer,
			const uint8_T *Cal_alt,
			const real_T *T_0,
			const uint8_T *T_int,
			const real_T *P_RAW,
			const real_T *deltaP_RAW,
			const int16_T *AB_T,
 ...
 *
 */
void send_MAVLink_v4_4_2_beta_3_HOME_sfun_Terminate_wrapper(void)
{
