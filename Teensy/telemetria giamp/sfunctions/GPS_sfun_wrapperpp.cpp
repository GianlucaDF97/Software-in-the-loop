...
/*  if (sizeof(double) < 8)
  {
    ...
    Serial2.println(F("Warning! Your platform does not support 64-bit double."));
    Serial2.println(F("The latitude and longitude will be inaccurate."));
  }*/
  ...
  Serial2.print("Current update rate: ");
  Serial2.println(rate);*/
...
/* %%%-SFUNWIZ_wrapper_Outputs_Changes_BEGIN --- EDIT HERE TO _END */
  ...
  if (millis() - lastTime > 1000)
  {
    ...
    int fixType = myGNSS.getFixType(); // returns the type of fix: 0=no, 1=1D, 2=2D, 3=3D, 4=GNSS+Deadreckoning
    int RTK=myGNSS.getCarrierSolutionType(); // Returns RTK solution: 0=no, 1=float solution, 2=fixed solution
   if  (fixType==4){
       fixType==6; //la regola solitamente dice che il fixType 6 corrisponde al dead reckoning, si è imposto questo if solo per farlo corrispondere.
   }
   else if (RTK==1){
       fixType==5;
   } 
   ...
   // Serial2.print(f_ellipsoid, 4); // Print the ellipsoid with 4 decimal places
    ...
    Serial2.print( d_fixType, 1); // Print the ellipsoid with 4 decimal places
    Serial2.println(" ");   
